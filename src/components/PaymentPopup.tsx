...
const VALID_COUPONS: Coupon[] = [
  ...
  },
  {
    ...
    type: "percentage",
    minAmount: 10000,
    description: "50% off for orders above ₹10,000",
  },
];
...
const PaymentPopup: React.FC<PaymentPopupProps> = ({
  ...
  const upiId = "9710404619@hdfc";
  const apiBase = import.meta.env.VITE_API_BASE_URL.replace(/\/+$/, "");

  // Use Indian price as primary payment amount
  const indianPriceStr = selectedPlan.priceIndia || "₹0";
  const originalPrice =
    parseInt(indianPriceStr.replace(/[₹,\-]/g, "").trim()) || 0;
  ...
  const handleApplyCoupon = async () => {
    ...
    if (coupon.minAmount && originalPrice < coupon.minAmount) {
      setCouponError(
        `Minimum order of ₹${coupon.minAmount.toLocaleString()} required`,
      );
      setCouponLoading(false);
      return;
    }
  ...
  const handleConfirmPayment = async () => {
    ...
    try {
      const { data: existing } = await axios.get(
        `${apiBase}/payments?transactionId=${normalizedId}`,
      );
      ...
      const paymentData = {
        ...
        storage: selectedPlan.storage || "N/A",
        price: `₹${finalPrice.toLocaleString()}`, // Final payable
        originalPrice: indianPriceStr,
        foreignPrice: selectedPlan.priceForeign,
      ...
      await emailjs.send(
        ...
        "template_yfrfahd",
        {
          ...
          orders: [
            {
              ...
              units: 1,
              price: finalPriceStr,
              originalPrice: indianPriceStr.replace(/[₹,\-]/g, "").trim(),
          ...
          cost: {
            ...
            tax: 0,
            total: finalPriceStr,
            originalTotal: indianPriceStr.replace(/[₹,\-]/g, "").trim(),
      ...
      <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-md flex items-center justify-center px-4 py-6">
        <div className="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-2xl w-full max-w-md max-h-[90vh] overflow-y-auto relative border border-gray-700 text-white p-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3">
              ...
              {!appliedCoupon ? (
                <motion.button
                  ...
                  className="px-6 py-3 rounded-xl bg-gradient-to-r from-green-400 to-green-500 text-white font-semibold text-sm shadow-lg transition disabled:opacity-50"
                >
                  {couponLoading ? (
                    <motion.span
          ...
          <div className="bg-[#131A22] rounded-xl p-4 mb-6 border border-gray-700">
            <h3 className="text-yellow-400 font-bold text-sm mb-4">
              Order Summary
            </h3>
            ...
            <div className="flex justify-between text-sm text-gray-400 mb-1">
              ...
              <span className="text-white">{indianPriceStr}</span>
            </div>
            <div className="flex justify-between text-sm text-gray-500 mb-3">
              <span>Foreign Price:</span>
              <span>{selectedPlan.priceForeign}</span>
            </div>
          ...
          <input
            ...
            className="w-full px-4 py-3 mb-6 rounded-xl bg-gray-700 border border-gray-600 text-white placeholder-gray-400 focus:outline-none focus:border-yellow-400 focus:ring-2 focus:ring-yellow-400/30"
          />
          ...
          <button
            ...
            className="w-full py-4 bg-gradient-to-r from-yellow-400 to-orange-500 text-black font-bold text-lg rounded-xl hover:scale-105 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
          >
            {isSubmitting
              ? "Processing…"
